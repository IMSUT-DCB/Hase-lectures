...
const PATTERNS = [
  ...
  // 「N問中M問」。片側が ${...} でも、もう片方が数字なら直書きである。
  // 両側とも ${...} なら正しい書きかたなので、数字が 1 つも無い形は通す。
  { re: /\d+\s*問中\s*(?:\d+|\$\{[^}]*\})\s*問/g,  why: '注意書き・修了証の「N問中M問」（分子が数字）' },
  { re: /(?:\d+|\$\{[^}]*\})\s*問中\s*\d+\s*問/g,  why: '注意書き・修了証の「N問中M問」（分母が数字）' },
...
function scan(file) {
  ...
  readFileSync(file, 'utf8').split('\n').forEach((line, i) => {
    // 1 行につき 1 件だけ報告する。「5問中4問」は分子側・分母側の両方の規則に当たるので、
    // そのままだと同じ行が 2 回出て件数が水増しされる。直す場所は 1 箇所なので 1 件でよい。
    for (const { re, why } of PATTERNS) {
      re.lastIndex = 0;
      const m = re.exec(line);
      if (m) { hits.push({ file, line: i + 1, text: line.trim().slice(0, 120), found: m[0], why }); break; }
    }
  });
  return hits;
}
...
if (process.argv.includes('--selftest')) {
  const bad = ['<p>Interactive Quiz · 基本問題 全5問</p>',
               ...
               '<div><dt>得点</dt><dd>5問中${score}問正解</dd></div>'];
  const good = ['<p>Interactive Quiz · 基本問題 <span id="quiz-total"></span></p>',
                ...
                '<p class="notice" id="quiz-rule">1問1点。${QUIZ.length}問中${Math.ceil(QUIZ.length*0.8)}問以上正解で合格です。</p>',
